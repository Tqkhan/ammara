...
class Printing extends MY_Controller {
    ...
    public function index()
    {
        ...
        $this->data['title'] = 'Printing';
        if ( $this->permission['view_all'] == '1'){
            $this->data['orders'] = $this->printing_model->get_item();
        }
        elseif ($this->permission['view'] == '1') {
            $this->data['orders'] = $this->printing_model->get_item($this->id);
        }
        $this->data['permission'] = $this->permission;
        $this->load->template('printing/index',$this->data);
    }
    ...
    public function report_submit($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $id = $this->printing_model->insert('printing_report',$data);
            if ($id) {
                redirect('printing');
            }
        }
        $this->data['title'] = 'Submit Report';
        $this->load->template('printing/report',$this->data);
    }
