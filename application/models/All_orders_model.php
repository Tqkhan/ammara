...
class all_orders_model extends MY_Model{
	...
	{
		$this->db->select('w.id,i.Description as product_name,w.quantity')
				 ->from('work_orders w')
				 ->join('item i', 'i.id = w.Item_Code')
				 ->join('production_plan pp','pp.WO_no = w.id')
				 // ->join('production_flow pf','pf.plane_id = pp.id and pf.id = '.$flow_id)
	...
	{
		$this->db->select('br.*,w.id,i.Description as product_name,w.quantity,m.machine_Name,brp.*')
				 ->from('work_orders w')
				 ->join('item i', 'i.id = w.Item_Code')
				 ->join('batch_release br', 'br.wo_id = w.id')
				 ->join('batch_release_parameters brp', 'brp.batch_release_id = br.id' , 'left')
				 ->join('production_plan pp','pp.WO_no = w.id')
